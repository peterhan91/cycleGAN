...
# Networks
netG_A2B = Generator2Dto3D(opt.input_nc, opt.output_nc)
netG_B2A = Generator3Dto2D(opt.output_nc, opt.input_nc)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        logger.log({'loss_G': loss_G, 'loss_G_GAN': (loss_GAN_A2B + loss_GAN_B2A),
                    ...
                    images={'real_A': real_A, 'real_B': real_B[int(len(real_B)//2)], 'fake_A': fake_A, 
                    'fake_B': fake_B[int(len(fake_B)//2)]})
