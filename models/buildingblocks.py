...
def create_conv(in_channels, out_channels, kernel_size, kernel_type, order, padding):
    """
    ...
    Args:
        ...
        kernel_type (string): 2d or 3d kernel, e.g.
            '2d' -> 2D conv 
            '3d' -> 3D conv
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            raise ValueError(f"Unsupported layer type '{char}'. MUST be one of ['i', 'b', 'r', 'l', 'e', 'c']")
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, kernel_type='2d', order='icr', padding=1):
        super(SingleConv, self).__init__()

        for name, module in create_conv(in_channels, out_channels, kernel_size, kernel_type, order, padding=padding):
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, kernel_type='2d', order='icr', padding=1):
        ...
        self.add_module('SingleConv1',
                        SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, kernel_type, order,
                                   padding=padding))
        # conv2
        self.add_module('SingleConv2',
                        SingleConv(conv2_in_channels, conv2_out_channels, kernel_size, kernel_type, order,
                                   padding=padding))
...
class Skipconnection(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, kernel_type='3d',
        ...
        if kernel_type == '3d':
            self.conv = SingleConv(in_channels, out_channels, kernel_size=kernel_size, kernel_type=kernel_type, 
                                    order=order, padding=padding)
        if kernel_type == '2d':
...
class Upsampling(nn.Module):
    ...
    def __init__(self, transposed_conv, in_channels=None, out_channels=None, kernel_size=3,
                 kernel_type='2d', scale_factor=2, mode='nearest'):
        super(Upsampling, self).__init__()

        if transposed_conv:
            if kernel_type == '2d':
                self.upsample = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=kernel_size, stride=scale_factor,
                                                padding=1)
            elif kernel_type == '3d':
    ...
    def forward(self, x):
        output_size_ = x.size()[2:]
        output_size = [i * 2 for i in output_size_]
        ###### check!!!! ######
